...
*/
void cBonusBlock::dropTreasures(b2World *physics_world, std::vector <cTreasure> &treasure)
{
	srand(time(NULL));

	for (int i = 0; i <= rand () % (max_treasures - min_treasures); i++)
		treasure.push_back(cTreasure(physics_world, sf::Vector2f(this->getPosition().x + rand() % 32, this->getPosition().y + rand() % 32), true, (float)((rand() % 2 ? -1 : 1) * rand() % 41 + 9) / 10, (float)((rand() % 2 ? -1 : 1) * rand() % 41 + 9) / 10));
}
